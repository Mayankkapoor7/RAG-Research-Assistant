...
MAX_CONTEXTS        = 5 # Max 5 groups after flteration
GOLDENS_PER_CONTEXT = 2 # 2 goldens per context
...
# Uisng this we get the two missing things for a test case creation
...
def main() -> None:
    ...
    test_cases = []
    # Passing a fresh session id for each question
    for pair in pairs:
    ...
    ) # Max 3 test cases, throttle_value is the waiting time between test cases

    summary = []
    # This is where we print the results
    for test_result in results.test_results:
        summary.append({
            ...
            "metrics": [
                {
                    "name": m.name, # Metric name
                    "score": m.score, # Score of the metric
                    "passed": m.success, # Whether the metric passed
                    "reason": m.reason, # Reason for the metric
                }
